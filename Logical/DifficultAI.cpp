...
{
    if (abs(zhadanx - xingyingx) <= 1 && abs(zhadany - xingyingy) <= 1)
    {
       // std::cout<<"zhandanx-xingyingx"<<abs(zhadanx - xingyingx);
        return 1;
    }
    else
    {
    ...
    if (abs(0 - enemyX2) + abs(3 - enemyY2) > 5)//5步以内不存在
    {
        if (zhaDanX != -1)//如果有炸弹
        {
            ...
            for (xyi; xyi < 5; xyi++)
            {
                if (xyi > -1)
                {
                    std::cout<<"tttttttttttttttt"<<isValidMove(x[xyi], y[xyi], nowBoard, nowBoard[zhaDanX][zhaDanY], zhaDanX, zhaDanY);
                    std::cout<<"bbbbbbbbbbbbbbbbbbb"<<whexingying(zhaDanX,zhaDanY,x[xyi],y[xyi]);
                    std::cout<<"xyi"<<xyi;
        ...
        for (int jump = 2; jump <= jumpMax; jump++) {
            ...
            if (nx >= 0 && nx < 12 && ny >= 0 && ny < 5) {
                if (isValidJumpMove(nx, ny, nowBoard, currentLevel, x, y)) {
                    ...
                    if( nowBoard[nx][ny]>14 && nowBoard[nx][ny] <= 23){
                        if (nowBoard[nx][ny] - 13 > currentLevel){
